...
// #define DF_EXCEPTION 1
// #define UD_EXCEPTION 6


void _interrupt db_handler() {
	printf("Debug Exception handled!\n");
}

void _interrupt ud_handler() {
	printf("UD exception handled\n");
...
void rewrite_ud_exception(PSYSINFO sysinfo){
    PIDTENTRY idt_table = (PIDTENTRY)sysinfo->idt.base;
...
void task_3() {
    ...
    PIDTENTRY idt_table = (PIDTENTRY)sysinfo->idt.base;
    uint32 old_offset = idt_table[DF_EXCEPTION].offset_h << 16 | idt_table[DF_EXCEPTION].offset_l;
    uint16 old_segment = idt_table[DF_EXCEPTION].seg_sel;
    uint32 new_offset;
    uint16 new_segment;

    __asm {
        mov edx, offset db_handler
        mov new_offset, edx
        mov ax, seg db_handler
        mov new_segment, ax
    }

    printf("Setting #DB handler\n");
    
    idt_set_gate(idt_table, DF_EXCEPTION, (uint32)new_offset, new_segment, idt_table[DF_EXCEPTION].flags);
    
    printf("#DB handler has been set\n");
